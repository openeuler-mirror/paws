...
SCHEDULER_ROOT=$(readlink -f "$KUBE_ROOT")
PAWS_ROOT="$(dirname $(readlink -f "$KUBE_ROOT"))"

# This variable can be injected, as in the verify script.
LICENSE_ROOT="${LICENSE_ROOT:-${PAWS_ROOT}}"
cd "${LICENSE_ROOT}"
...
# copy licenses for forked code from vendor directories
(cd "${KUBE_ROOT}" && \
  find vendor -iname 'licen[sc]e*' -o -iname 'notice*' -o -iname 'copying*' | \
  grep -E 'forked' | \
